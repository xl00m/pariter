    ...
    if (req.method === 'POST' && path === '/api/ai/rewrite') {
      ...
      const systemPrompt = (field === 'victory')
        ? [
            ...
            '',
            'Ты — модуль кристаллизации успеха в системе Pariter. Твоя задача — принять сырую мысль пользователя о его достижении (блока VICTORIA) и очистить её от шума, придав форму и вес.',
            '',
            'Правила обработки:',
